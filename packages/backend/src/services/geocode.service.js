...
function shortenDisplayName(displayName) {
    const parts = displayName
        ...
        .map((s) => s.trim())
        .filter(Boolean);
    const skip = (p) => /^\d{5}(-\d{4})?$/.test(p) || /україна|ukraine|украина|ukr\b/i.test(p) || /^область\b/i.test(p);
    const filtered = parts.filter((p) => !skip(p));
    return filtered.slice(0, 3).join(', ') || displayName;
}
function nominatimAddressToShortLabel(addr) {
    const a = addr;
    ...
    if (road && hn)
        ...
        line1 = a.amenity || a.building || a.shop || '';
