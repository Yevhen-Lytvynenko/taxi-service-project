...
exports.analyzeAndRankRoutes = analyzeAndRankRoutes;
...
async function buildSpeedHeatmap() {
    const now = Date.now();
...
async function countBusyDriversInCorridor(coords, corridorMeters = 600) {
    if (coords.length < 2)
        return 0;
    ...
    const busy = await prisma_1.prisma.driverProfile.findMany({
        where: {
            status: 'BUSY',
    ...
    for (const d of busy) {
        ...
        for (let i = 0; i < coords.length; i++) {
            const c = coords[i];
            const dist = (0, route_service_1.haversineMeters)([c[0], c[1]], [d.currentLng, d.currentLat]);
            if (dist < minDist)
                minDist = dist;
...
function estimateRouteSpeedFromHeatmap(coords, heatmap) {
    if (coords.length < 2)
        return { avgSpeedKmh: 30, slowSegments: 0, coveredCells: 0 };
    let totalWeight = 0;
    let weightedSpeed = 0;
    let slowSegments = 0;
    let coveredCells = 0;
...
async function analyzeRoute(option) {
    const heatmap = await buildSpeedHeatmap();
    const { avgSpeedKmh, slowSegments, coveredCells } = estimateRouteSpeedFromHeatmap(option.coordinates, heatmap);
    ...
    const heatmapSpeedFactor = coveredCells >= 3 && avgSpeedKmh < 18 ? 1.22 :
        coveredCells >= 3 && avgSpeedKmh < 25 ? 1.10 : 1.0;
    const adjustedDurationSec = Math.round(baseSec * timeFactor.factor * congestionPenalty * slowPenalty * heatmapSpeedFactor);
    const congestionLevel = busyDrivers >= 5 || slowSegments >= 4 ? 'high' :
    ...
    const score = Math.round(timeScore * 0.40 +
        distScore * 0.20 +
        congScore * 0.25 +
        speedScore * 0.15);
    const insights = [];
...
async function analyzeAndRankRoutes(options) {
    if (options.length === 0) {
    ...
    const ranked = scored.map(({ opt, analysis }, i) => {
        ...
        const congLabel = analysis.congestionLevel === 'low' ? '🟢' :
...
//# sourceMappingURL=trafficAnalytics.service.js.map